      ...
      <el-upload
        ...
        <template #trigger>
          <div class="upload-header">
            <span class="prompt-text" style="font-weight: bold">请选择要导入的文件：</span>
...
function handleBeforeUpload(file) {
  ...
  if (file.name.lastIndexOf(".") > -1) {
    ...
    isExcel = ["xls", "xlsx"].some(type => file.type.indexOf(type) > -1 || fileExtension.indexOf(type) > -1);
  }
  if (!isExcel) {
    proxy.$modal.msgError("文件格式不正确，请上传xls/xlsx格式文件!");
    return false;
  }
  if (file.size / 1024 / 1024 > 1) {
    proxy.$modal.msgError("上传excel大小不能超过1MB!");
    return false;
  }
